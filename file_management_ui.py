...
from accounting_utils import get_trial_balance, get_income_statement_data, get_balance_sheet_data, get_account_entries
    ...
    try:
        ...
        if journal_data:
            ...
            for row_num in range(len(journal_data)):
                worksheet.write(row_num + 2, 4, journal_data[row_num][t("amount")], currency_format)
        ...
        for account in accounts:
            ...
            starting_balance = getattr(account, 'balance', 0.0) if account.category in ["Active", "Passive"] else 0.0
            ...
            if not account_entries_df.empty:
                ...
                entries_data = entries_data.rename(columns={
                    "Date": t("date"),
                ...
                for _, row in entries_data.iterrows():
                    account_detail_data.append({
                        t("date"): row[t("date")].strftime("%Y-%m-%d") if hasattr(row[t("date")], 'strftime') else str(row[t("date")]),
                ...
                for row_idx, (_, row_data) in enumerate(detail_df.iterrows()):
                    ...
                    if row_data[t("date")] and row_data[t("date")] != t("starting_balance"):
                        try:
                            ...
                            worksheet.write_datetime(actual_row, 0, date_obj, date_format)
                        except:
        ...
        result_worksheet.write(max_detail_row, 0, f"{t('total')} {t('products')}", total_format)
        result_worksheet.write(max_detail_row, 1, income_data.get("total_products", 0), total_format)
        
        # Expenses total (aligned)
        result_worksheet.write(max_detail_row, 3, f"{t('total')} {t('expenses')}", total_format)
        result_worksheet.write(max_detail_row, 4, income_data.get("total_expenses", 0), total_format)
        ...
        result_worksheet.merge_range(f'A{net_income_row + 1}:B{net_income_row + 1}', f"{t('net_income')}: {income_data.get('net_income', 0):.2f}", total_format)
        ...
        balance_worksheet.write(max_detail_row, 0, f"{t('total')} {t('active')}", total_format)
        balance_worksheet.write(max_detail_row, 1, balance_data.get("total_active", 0), total_format)
        
        # Passive total (aligned)
        balance_worksheet.write(max_detail_row, 3, f"{t('total')} {t('passive')}", total_format)
        balance_worksheet.write(max_detail_row, 4, total_passive_with_income, total_format)
        ...
        if abs(balance_diff) < 0.01:  # Allow for small rounding differences
            balance_status = f"✓ {t('balance_verified')}: {balance_data.get('total_active', 0):.2f}"
        ...
        balance_worksheet.merge_range(f'A{balance_status_row + 1}:E{balance_status_row + 1}', balance_status, balance_status_format)
...
def show_file_management_dialog():
    ...
    with tab4:
        ...
        # Create Excel export
        if st.button(t("export_as_excel"), type="primary", key="export_excel", use_container_width=True):
            try:
